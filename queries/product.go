...
func BuildProductPTCBQuery(filterBy string, pageSize int, offset int) string {
	...
	baseQuery := `
        ...
        WHERE %s = $1
		ORDER BY p.discount DESC, p.status='available' DESC, p.status='comingSoon' DESC
		LIMIT %d OFFSET %d;
		`

	return fmt.Sprintf(baseQuery, column, pageSize, offset)
}
